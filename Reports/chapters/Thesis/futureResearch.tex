...
Based on the conclusions (Chapter \ref{chap:conclusion}) certain areas of improvement have been identified, with regards to existing standards and linked data ontologies and \ac{sparql} endpoints. This chapter presents five recommendations, which enable the presented conceptual system architecture (Section \ref{recommend}) to work with any \ac{sos} and to be completely automated. Due to the limited scope of thesis, there are also still three remaining topics for future work. These are presented in Section \ref{FW}.
