...
\begin{figure}
	...
	\caption{Workflow diagram of Web Processing Service for creating linked sensor metadata (adapted from \cite{LD:Missier})}
...
\begin{figure}
	...
	\caption{Workflow diagram of Web Processing Service for retrieving sensor data}
...
for i, procedure in enumerate(SOSclass.procedures):
	...
	g.add( ( procedureURI, RDF.type, om_lite.process) )
...
\begin{lstlisting}[caption={Serializing the RDFlib graph object and posting it to the Strabon endpoint}, label={lst:rdflib}]
import os
import requests
import rdflib
...
\begin{lstlisting}[caption={Example of a PURL batch file (containing one PURL)}, label={lst:PURL}]
<purls>
...
\begin{lstlisting}[caption={Example script that sends a SPARQL query for retreiving geometries of input features}, label={lst:getGeometries}]
import requests 
...
\begin{lstlisting}[caption={A spatial SPARQL query for discovering sensors and their SOS related metadata}, label={lst:getSensors}]
...
\begin{lstlisting}[caption={Script that creates an LXML graph object called spatialFilter to add to a \texttt{GetObservaton} POST request}, label={lst:getObservation}]
...
\begin{lstlisting}[caption={Script that performs basic temporal aggregation methods on a comma separated value string}, label={lst:aggregate}]
import numpy
