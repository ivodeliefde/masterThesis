...
\label{chap:rw}
To answer the research questions of this thesis an understanding of the current state of the art in sensor web research is required. A number of topics are relevant for this, such as existing \ac{swe} standards for publishing sensor data to the web (Paragraph \ref{par:SWE}), the development of the semantic web (Paragraph \ref{par:SemanticWeb}) and linked data ontologies for observation data (Paragraph \ref{par:ontologies}) and their relation to developments in the \ac{iot} (Paragraph \ref{par:SSWIOT}). Furthermore, extensions to the \ac{ogc} catalog service are described for including sensor metadata (Paragraph \ref{par:catalogService}), as well as methods for adding semantics to \ac{swe} data services (Paragraph \ref{par:middleware}). This chapter discusses the recent relevant literature on these topics. 
...
\begin{figure}
	...
	\caption{\texttt{DescribedObject} class in \ac{sensorml}. Image courtesy by \cite[p. 39]{SW:OGC7}}
...
\begin{figure}
	...
	\caption{Definition of a physical process in \ac{sensorml}. Image courtesy by \cite[p. 57]{SW:OGC7}}
...
\begin{figure}
	...
	\caption{Data model behind the capabilities document of a \ac{sos}. Image courtesy by \cite{SW:OGC2}}
...
The \ac{ogc} has developed a way of including sensors into a catalogue service using sensor registries. The different services related to this are described, starting with the \acf{csw} (Paragraph \ref{par:CSW}), after which the \acf{sor} (Paragraph \ref{par:SOR}) and \acf{sir} (Paragraph \ref{par:SIR}) services will be presented.

\begin{figure}
	\centering
	\includegraphics[width=0.6\linewidth]{UML/CSW.png}
	\caption{Architecture of \acl*{csw}. Image courtesy by \cite[p. 26]{GEO:OGC2}}
	\label{fig:CSW}
\end{figure}
...
\begin{figure}
	...
	\caption{Overview of the interaction between CSW, SIR, SOR and potential users. Image courtesy by \cite{SW:52North2}}
	\label{fig:SorSir}
\end{figure}
...
The semantic web is based on the open world assumption. This means that \enquote{the thruth of a statement is independent of whether it is known} \citep[p. 103]{LD:Hebeler}. Traditional software application use the closed world assumption. Here a statement is assumed to be false if the answer is unknown. A simple example of this is a database table containing costumers of a store. If a person's name is not in that table the closed world assumption decides that this person is not a costumer of the store. The name is unknown and hence the statement `he/she is a customer' is false. An open world assumption would decide that it is simply unknown whether this person is a costumer, since it has not been described anywhere. The closed world assumption only works in an environment where all information is complete. There should be no possibility in our example that people who are customers are not present in the database table. The internet is an enormous collection of data stored in different places and it can never be guaranteed that this data is complete. Therefore the semantic web is based on the open world assumption \citep{LD:Hebeler}. 

In the next paragraphs the framework of the semantic web --  the \acf{rdf} -- will be described (Paragraph \ref{par:rdf}), followed by different \ac{rdf} notations (Paragraph \ref{par:notation}). The last part explains the concept of \acp{purl} (Paragraph \ref{par:purl}).
...
\begin{figure}
	...
	\caption{Persistent Uniform Resource Locator (PURL) resolves to the current resource location. Image courtesy by \cite{LD:PURL}}
	\label{fig:PURL}
\end{figure} 
...
\section{Sensor web and the Internet of Things}
\label{par:SSWIOT}

A growing field of research is evolving around the \acf{iot}. Sensor and network technology are becoming cheaper and the required hardware increasingly smaller. This allows them to be used for things that would traditionally not produce any sensor data at all, such as buildings, street lights and consumer electronics. Companies operating in a variety of different markets are investing in this development, accelerating the creation of an \ac{iot} \citep{IoT:PWC}.    

\cite{IOT:Barnaghi} point out that the heterogeneity of `things' makes interoperability among \ac{iot} devices a challenging problem. They explain that the semantic web could be of great importance for the description of \ac{iot} devices and their observations. However, \enquote{providing automated or semi-automated methods and tools to annotate, publish and access the semantic descriptions} is still described as a topic for future research \cite[p. 19]{IOT:Barnaghi}.

Another study on the crossroads between the \ac{iot} and \ac{swe} has been performed by \cite{IOT:Jazayeri}. They have analysed and evaluated four interoperable open standards for the \ac{iot}: PUCK, TinySOS, \ac{sos}, and the SensorThings \ac{api}. They found that all four open standards are useful for \ac{iot}, but that they all come with their own negative and positive attributes. This makes the decision for one of these standard dependent on the application for which they going to being used for. For memory constraint applications PUCK and TinySOS are the preferred candidates, while \ac{sos} (in combination with the the coap protocol) and the sensorThings \ac{api} perform better in terms of bandwidth efficiency \citep{IOT:Jazayeri}. 
