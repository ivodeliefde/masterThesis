...
The \texttt{GetCapabilities} request is the first step in communicating with a \ac{sos}. It returns a document including information on what it has to offer. The structure contains a number of sections: service identification, service provider, operations metadata, filter capabilities and contents. 
...
\texttt{DescribeSensor} gives detailed information on a specific sensor.

\subsection{Get observation}

Using \texttt{GetObservation} actual measurements can be retrieved. 
